...
/* 遥控 */
#define SPEED_Y 550         //俯仰(前后)最大设定速度
...
void car_control(void)
{
    ...
    //遥控
    /* 前后 */
    if((g_forw==0)&&(g_back==0))g_target_speed = 0;//未接受到前进后退指令-->速度清零，稳在原地
    if(g_forw==1)                   //前进
    {
//        if(g_distance < 50)         //超声波避障，根据实际情况调整正负号
//            g_target_speed -= 10;
//        else
            g_target_speed += 10;
    }
    if(g_back==1)g_target_speed -= 10;//后退
    g_target_speed = g_target_speed>SPEED_Y ? SPEED_Y : (g_target_speed<-SPEED_Y ? (-SPEED_Y) : g_target_speed);//限幅

    /* 左右 */
    if((g_left==0)&&(g_right==0))g_target_turn=0;
    if(g_left==1)g_target_turn -= 10; //左转
    if(g_right==1)g_target_turn += 10;//右转
    g_target_turn=g_target_turn>SPEED_Z?SPEED_Z:(g_target_turn<-SPEED_Z?(-SPEED_Z):g_target_turn);//限幅( (20*100) * 100   )

    /* 转向约束 */
    if((g_left==0)&&(g_right==0))turn_Kd=-0.3;   //若无左右转向指令，则开启转向约束
    else if((g_left==1)||(g_right==1))turn_Kd=0;//若左右转向指令接收到，则去掉转向约束
