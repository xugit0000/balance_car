...
{
    if(GPIO_Pin == MPU_INT_Pin)          //陀螺仪中断引脚，周期10ms，执行pid控制
    {
        /* 释放信号量，唤醒高优先级任务 */
        static BaseType_t xHigherPriorityTaskWoken;
        xSemaphoreGiveFromISR(g_binsemphore_mpu6050,&xHigherPriorityTaskWoken);
        portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
    }
//    if(GPIO_Pin == ECHO_Pin)              //超声波接收引脚，测距，加入FreeRTOS定时器3被占用，无法使用超声波
//    {
//        sr04_callback();
//    }
//    if(GPIO_Pin == MPU_INT_Pin)          //陀螺仪中断引脚，周期10ms，执行pid控制
